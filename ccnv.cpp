...
int main(int argc, char * argv[])
{
        if(argc==1)
        {
		cerr<<"Usage: "<<argv[0]<<" -d1 delta_candidate.out -d2 delta_ref.out -q query_name.list -c cutoff_for_repeats -D master_delta.out"<<endl;
        	exit(EXIT_FAILURE);
        }

	ifstream Delta,delta1,delta2,fin;
	ofstream fout,chromMap,unmapped;	
...
//cout<<header<<endl;
	...
	for(map<string,int>::iterator it=merge.storeCount.begin();it!= merge.storeCount.end();it++)
	{
		...
		{
			unmapped<<it->first<<"\t"<<it->second<<"\t"<<merge1.storeCount[it->first]<<endl;
		}
